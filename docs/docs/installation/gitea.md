...
```bash
docker build -f /docker/Dockerfile -t pr-insight:gitea_app --target gitea_app .
docker push khulnasoft/pr-insight:gitea_webhook  # Push to your Docker repository
```
...
8. Create a webhook in your Gitea project. Set the URL to `http[s]://<PR_INSIGHT_HOSTNAME>/api/v1/gitea_webhooks`, the secret token to the generated secret from step 3, and enable the triggers `push`, `comments` and `merge request events`.
