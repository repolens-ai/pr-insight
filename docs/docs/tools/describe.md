...
By default, PR-Insight tries to preserve your original PR description by placing it above the generated content.
This requires including your description during the initial PR creation.

"PR-Insight removed the original description from the PR. Why"?
...
![Describe comment](https://khulnasoft.com/images/pr_insight/pr_description_user_description.png){width=512}
...
???+ example "Possible configurations"
    ...
    <table>
      ...
      <tr>
        ...
        <td>If set to true, it will add a comment message [`PR Description updated to latest commit...`](https://github.com/repolens-ai/pr-insight/pull/499#issuecomment-1837412176) after finishing calling `/describe`. Default is true.</td>
      </tr>
      <tr>
...
The marker `pr_insight:type` will be replaced with the PR type, `pr_insight:summary` will be replaced with the PR summary, `pr_insight:walkthrough` will be replaced with the PR walkthrough, and `pr_insight:diagram` will be replaced with the sequence diagram (if enabled).
