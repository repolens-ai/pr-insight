...
See [here](https://github.com/repolens-ai/pr-insight/blob/main/pr_insight/algo/__init__.py) for a list of supported models in PR-Insight.
The default model of PR-Insight is `GPT-5` from OpenAI.
To use a different model than the default, you need to edit in the [configuration file](https://github.com/repolens-ai/pr-insight/blob/main/pr_insight/settings/configuration.toml#L2) the fields:
...
By default, Ollama uses a context window size of 2048 tokens. In most cases this is not enough to cover pr-insight prompt and pull-request diff. Context window size can be overridden with the `OLLAMA_CONTEXT_LENGTH` environment variable. For example, to set the default context length to 8K, use: `OLLAMA_CONTEXT_LENGTH=8192 ollama serve`. More information you can find on the [official ollama faq](https://github.com/ollama/ollama/blob/main/docs/faq.md#how-can-i-specify-the-context-window-size).
...
!!! note "Local models vs commercial models"
    PR-Insight is compatible with almost any AI model, but analyzing complex code repositories and pull requests requires a model specifically optimized for code analysis.
...
Also, review the [AiHandler](https://github.com/repolens-ai/pr-insight/blob/main/pr_insight/algo/ai_handler.py) file for instructions on how to set keys for other models.
...
To use xAI's models with PR-Insight, set:
...
If the relevant model doesn't appear [here](https://github.com/repolens-ai/pr-insight/blob/main/pr_insight/algo/__init__.py), you can still use it as a custom model:
