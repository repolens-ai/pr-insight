...
from pr_insight.servers.github_app import handle_new_pr_opened
...
async def test_add_docs_trigger(monkeypatch, action, draft, state, should_run):
    ...
    monkeypatch.setattr(
        "pr_insight.git_providers.utils.get_git_provider_with_context",
        lambda pr_url: FakeGitProvider(pr_url),
    )
    monkeypatch.setattr(
        "pr_insight.tools.pr_add_docs.get_git_provider",
