...
from pr_insight.config_loader import apply_secrets_manager_config, apply_secrets_to_config


class TestConfigLoaderSecrets:

    def test_apply_secrets_manager_config_success(self):
        with patch('pr_insight.secret_providers.get_secret_provider') as mock_get_provider, \
             patch('pr_insight.config_loader.apply_secrets_to_config') as mock_apply_secrets, \
             patch('pr_insight.config_loader.get_settings') as mock_get_settings:
    ...
    def test_apply_secrets_to_config_nested_keys(self):
        with patch('pr_insight.config_loader.get_settings') as mock_get_settings:
            settings = MagicMock()
